...
class admin_frontend_setting_Controller extends Controller
{
    ...
    */
    ...
    ===============
    */
    // admin_category_all_controller
    public function admin_category_all_controller()
    {
        $dataType = Category::orderBy('order', 'ASC') -> paginate(10);
        return view('admin.pages.settings.category.index') -> with(compact('dataType'));
    }
    // admin_category_add_controller
    public function admin_category_add_controller()
    {
        return view('admin.pages.settings.category.add');
    }

    // admin_category_update_controller
    public function admin_category_update_controller($id)
    {
        $data = Category::where('id', $id) -> first();
        $urls = urls::where('category_id', $id) -> get();

        return view('admin.pages.settings.category.update') -> with(compact('id', 'data', 'urls'));
    }
